// #ifdef APP-PLUS 
import Http from '@/api/http/wx.js'
import globalConfig from '@/global/js/config'

const platform = uni.getSystemInfoSync().platform;
// 主颜色
const $mainColor = globalConfig.mainColor || "#077ff9";
// 弹窗图标url
const $iconUrl = globalConfig.iconUrl || "/static/update.png";
//点击模态框是否关闭
const dialogClose = globalConfig.dialogClose || false
//点击关闭后再次提醒时间（天）
const closeDay = globalConfig.closeDay || 7

const judgeTime = function() {
	let closeTime = uni.getStorageSync('closeTime')

	if (closeTime) {
		closeTime = new Date(closeTime + 1000 * 60 * 60 * 24 * closeDay)
...
const getServerNo = function(version, isPrompt = false, callback) {
	...
	if (judgeTime()) isPrompt(true);
	Http.http_normal({
		url: globalConfig.requestUrl,
		method: 'get'
	}).then(res => {
		if (res.data.version_code != httpData.version_code || res.data.version_name != httpData.version_name) {
			callback({
				versionCode: res.data.version_code,
				versionName: res.data.version_name,
				versionInfo: res.data.version_info,
				forceUpdate: res.data.force_update,
...
function updatePopup(data, callback) {
	...
	if (!data.forceUpdate) {
		...
		popupView.addEventListener("click", function(e) {
			...
			if (e.clientY > maxTop - 30 && e.clientY < maxTop) {
				...
				if (e.clientX > viewContentPadding && e.clientX < maxLeft - buttonWidth - viewContentPadding) {
					maskLayer.hide();
					popupView.hide();
					uni.setStorageSync('closeTime', (new Date).getTime())
		...
		popupView.addEventListener("click", function(e) {
			let maxTop = popupViewHeight - viewContentPadding;
			let maxLeft = popupViewWidth - viewContentPadding;
			let buttonWidth = viewContentWidth;
			if (e.clientY > maxTop - 35 && e.clientY < maxTop) {
				if (e.clientX > viewContentPadding && e.clientX < popupViewHeight - viewContentPadding) {
					// 立即升级
					maskLayer.hide();
					popupView.hide();
					callback && callback();
